...
export default function EventsFinanceClientPage() {
    ...
    const { data: events, setData: setEvents } = useCommunityEventsData();
    const { data: finances, setData: setFinances } = useCommunityFinancesData();
    ...
    const { totalIncome, totalExpense, netBalance } = useMemo(() => {
        const income = filteredFinances.filter(f => f.type === 'Income').reduce((sum, item) => sum + item.amount, 0);
        const expense = filteredFinances.filter(f => f.type === 'Expense').reduce((sum, item) => sum + item.amount, 0);
     ...
     const handleSaveTransaction = (values: TransactionValues, id?: string) => {
        if (id) {
            setFinances(prev => prev.map(f => f.id === id ? { ...f, ...values } : f));
