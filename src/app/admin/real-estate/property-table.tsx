...
const AddEditPropertyDialog = ({ 
    ...
    property?: Property, 
    onSave: (values: z.infer<typeof PropertySchema>, id?: string) => void,
    ...
    useEffect(() => {
        if(isOpen) {
           form.reset(property || { title: "", listingType: "For Sale", propertyType: "Villa", location: "", price: 0, bedrooms: 3, bathrooms: 4, areaSqM: 300, description: "", status: "Available", buildingAge: "New", imageUrl: "https://picsum.photos/seed/newprop/600/400", aiHint: 'modern house exterior' });
        }
    ...
    const onSubmit: SubmitHandler<z.infer<typeof PropertySchema>> = async (data) => {
...
export default function PropertyTable({ properties, setProperties, isClient }: { properties: Property[], setProperties: (updater: (properties: Property[]) => void) => void, isClient: boolean }) {
    ...
    const handleSave = (values: z.infer<typeof PropertySchema>, id?: string) => {
        if (id) {
