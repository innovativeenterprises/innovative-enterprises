...
import { Loader2, Sparkles, Check, TrendingUp, X, Banknote, MapPin, Building, BedDouble, Bath, Wand2, FileCheck2 } from 'lucide-react';
  ...
  const handleFloorPlanAnalysis = async () => {
    ...
    try {
        ...
        if (result.projectType) {
            // Map the result to a valid enum value for the valuation form
            let mappedType: 'Villa' | 'Apartment' | 'Townhouse' | 'Commercial Space' = 'Villa';
            if (result.projectType.includes('Villa')) mappedType = 'Villa';
            if (result.projectType.includes('Commercial')) mappedType = 'Commercial Space';
            form.setValue('propertyType', mappedType);
        }
        if (result.numberOfFloors) {
            // A simple heuristic for bedrooms/bathrooms based on floors
            form.setValue('bedrooms', result.numberOfFloors * 2);
            form.setValue('bathrooms', result.numberOfFloors * 2 + 1);
        }
        toast({ title: 'Floor Plan Analyzed', description: 'AI has pre-filled some of the project details for you.' });
    } catch (e) {
        toast({ title: 'Analysis Failed', description: 'Could not analyze the floor plan. Please enter details manually.', variant: 'destructive' });
    } finally {
    ...
    <div className="space-y-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Provide as much detail as possible for the most accurate valuation. You can upload a floor plan to let the AI pre-fill some fields.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel>Floor Plan Document (Optional)</FormLabel>
                            ...
                            <FormDescription>Upload a floor plan, and our AI will attempt to pre-fill the form.</FormDescription>
                            <FormMessage />
                        </FormItem>
                    )}
                />
                 {analysis && (
                    <Alert>
                        ...
                        <AlertDescription>
                            <p>Successfully pre-filled some details based on the plan. Please review and adjust as needed.</p>
                        </AlertDescription>
                    </Alert>
                )}
                ...
                />
              <Button type="submit" disabled={isLoading || isAnalyzingPlan} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground text-base" size="lg">
                {isLoading ? (
