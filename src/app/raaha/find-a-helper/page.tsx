...
const WorkerCard = ({ worker }: { worker: Worker }) => {
    return (
        <Link href={`/raaha/worker/${worker.id}`}>
            <Card className="overflow-hidden group transition-all duration-300 hover:shadow-xl flex flex-col h-full">
            ...
            {response && (
                <div>
                     ...
                     <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {response.recommendedWorkers.map(worker => <WorkerCard key={worker.id} worker={worker} />)}
                    </div>
                </div>
            )}
        </div>
