...
export default function LogisticsSchedulerForm() {
  ...
  const onSubmit: SubmitHandler<TimetableGeneratorInput> = async (data) => {
    ...
    try {
      ...
      if (result.diagnostics.isPossible) {
          toast({ title: 'Schedule Generated!', description: 'Your optimized delivery schedule is ready.' });
      } else {
  ...
  return (
    <div className="space-y-8">
      <Card>
        <CardHeader>
          <CardTitle>Define Your Logistics Constraints</CardTitle>
          <CardDescription>Define your delivery tasks, vehicles, and destinations. Our AI will generate an optimized, conflict-free weekly schedule.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold">Delivery Tasks & Vehicles</h3>
                        {subjectFields.map((field, index) => (
                             <Card key={field.id} className="p-4 bg-muted/50 relative">
                                <div className="grid grid-cols-2 gap-4">
                                     <FormField control={form.control} name={`subjects.${index}.name`} render={({ field }) => (<FormItem><FormLabel>Task / Route Name</FormLabel><FormControl><Input placeholder="e.g., Delivery to Sohar" {...field} /></FormControl><FormMessage/></FormItem>)} />
                                     <FormField control={form.control} name={`subjects.${index}.teacher`} render={({ field }) => (<FormItem><FormLabel>Assigned Vehicle</FormLabel><FormControl><Input placeholder="e.g., Truck A (10-ton)" {...field} /></FormControl><FormMessage/></FormItem>)} />
                                </div>
                         ...
                         <Button type="button" variant="outline" size="sm" onClick={() => appendSubject({ id: `task_${Date.now()}`, name: '', teacher: '', requiredSlots: 2 })}><PlusCircle className="mr-2 h-4 w-4"/> Add Task</Button>
                    </div>
                     {/* Job Sites */}
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold">Destinations / Hubs</h3>
                        {classroomFields.map((field, index) => (
                             <Card key={field.id} className="p-4 bg-muted/50 relative">
                                <FormField control={form.control} name={`classrooms.${index}.name`} render={({ field }) => (<FormItem><FormLabel>Destination Name</FormLabel><FormControl><Input placeholder="e.g., Sohar Port" {...field} /></FormControl><FormMessage/></FormItem>)} />
                         ...
                         <Button type="button" variant="outline" size="sm" onClick={() => appendClassroom({ id: `site_${Date.now()}`, name: '' })}><PlusCircle className="mr-2 h-4 w-4"/> Add Destination</Button>
                    </div>
                </div>
              <Button type="submit" disabled={isLoading} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground text-base" size="lg">
                {isLoading ? (
                  ...
                  <><Sparkles className="mr-2 h-4 w-4" /> Generate Delivery Schedule</>
                )}
              </Button>
            </form>
          </Form>
      ...
      {response?.diagnostics && response.diagnostics.isPossible && (
        <Card className="mt-8">
          <CardHeader>
            <CardTitle>Generated Weekly Delivery Schedule</CardTitle>
