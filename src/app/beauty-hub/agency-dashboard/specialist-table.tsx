...
export function SpecialistTable({ agencyId }: { agencyId: string }) {
    const { specialists, setSpecialists, isClient } = useBeautySpecialistsData();
    const { toast } = useToast();
    
    const filteredSpecialists = specialists.filter(s => s.agencyId === agencyId);
    ...
    const handleSave = (values: SpecialistValues, id?: string) => {
        if (id) {
            ...
            const newSpecialist = { ...values, id: `spec_${Date.now()}`, agencyId };
