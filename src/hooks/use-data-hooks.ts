...
// A pattern for creating hooks that manage a specific slice of the global state.
// This helps keep components clean and focused on their own data needs.
const createDataHook = <T,>(
  sliceSelector: (state: any) => T[],
  initialData: T[] = []
) => {
  return (initialDataOverride?: T[]) => {
    const dataSlice = useGlobalStore(sliceSelector);
    const setStore = useGlobalStore(s => s.set);
    const isClient = useGlobalStore(s => s.isClient);

    const [data, setData] = useState(initialDataOverride || initialData);
    ...
    useEffect(() => {
        setStore(state => ({ ...state, [sliceSelector.toString().split('.')[1]]: data }));
    }, [data, setStore, sliceSelector]);
...
export const useStaffData = (initialData: any = { leadership: [], staff: [], agentCategories: [] }) => useGlobalStore(state => ({ leadership: state.leadership, staff: state.staff, agentCategories: state.agentCategories, set: state.set, isClient: state.isClient }));
...
export const useRaahaRequestsData = (initialData: any[] = []) => useGlobalStore(state => ({ data: state.raahaRequests, setData: state.set, isClient: state.isClient }));
