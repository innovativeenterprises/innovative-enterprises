...
export function useCommunityHubData() {
    ...
    return { events, finances, isClient: true };
};

export function useMembersData() {
    const members = useStoreData(state => state.communityMembers);
    return { members, isClient: true };
};
...
export function usePropertiesData() {
    ...
    return { properties, isClient: true };
};
...
export const setAgencies = (updater: (agencies: Agency[]) => Agency[]) => store.set(state => ({ ...state, raahaAgencies: updater(state.raahaAgencies) }));
export const setWorkers = (updater: (workers: RaahaWorker[]) => RaahaWorker[]) => store.set(state => ({ ...state, raahaWorkers: updater(state.raahaWorkers) }));
