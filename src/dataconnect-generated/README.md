...
## InsertOrganization
You can execute the `InsertOrganization` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
insertOrganization(): MutationPromise<InsertOrganizationData, undefined>;
...
```typescript
insertOrganization(dc: DataConnect): MutationPromise<InsertOrganizationData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the insertOrganizationRef:
```typescript
const name = insertOrganizationRef.operationName;
console.log(name);
```
...
### Return Type
Recall that executing the `InsertOrganization` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `InsertOrganizationData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface InsertOrganizationData {
  organization_insert: Organization_Key;
}
```
### Using `InsertOrganization`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, insertOrganization } from '@dataconnect/generated';


// Call the `insertOrganization()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await insertOrganization();

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await insertOrganization(dataConnect);

console.log(data.organization_insert);

// Or, you can use the `Promise` API.
insertOrganization().then((response) => {
  const data = response.data;
  console.log(data.organization_insert);
});
```

### Using `InsertOrganization`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, insertOrganizationRef } from '@dataconnect/generated';


// Call the `insertOrganizationRef()` function to get a reference to the mutation.
const ref = insertOrganizationRef();

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = insertOrganizationRef(dataConnect);
